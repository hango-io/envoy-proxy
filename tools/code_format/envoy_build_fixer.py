...
API_INCLUDE_REGEX = re.compile(
    '#include "(contrib/envoy/.*|envoy/.*)/[^/]+\.pb\.(validate\.)?h"')
...
def fix_package_and_license(path, contents):
    ...
    envoy_bazel_prefix = os.getenv("ENVOY_BAZEL_PREFIX", "")
    if envoy_bazel_prefix == "":
    ...
    if re.search(ENVOY_RULE_REGEX, contents):
        ...
        contents = run_buildozer([
            (new_load.format(envoy_bazel_prefix), '__pkg__'),
        ], contents)
...
def fix_api_deps(path, contents):
    ...
    for line in buildozer_out.split('\n'):
        ...
        if srcs != 'missing':
            source_paths.extend(
                os.path.join(source_dirname, f) for f in srcs.split()
                if f.endswith('.cc') or f.endswith('.h'))
        if hdrs != 'missing':
            source_paths.extend(
                os.path.join(source_dirname, f) for f in hdrs.split()
                if f.endswith('.h'))
        api_hdrs = set([])
