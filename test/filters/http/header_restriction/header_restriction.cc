...
public:
  ...
  void SetUpRouteTest(const std::string& yaml) {
    ProtoRouteConfig proto_config;
    TestUtility::loadFromYaml(yaml, proto_config);

    route_config_ = std::make_shared<RouteConfig>(proto_config);

    ON_CALL(*decoder_callbacks_.route_,
            mostSpecificPerFilterConfig("proxy.filters.http.header_restriction"))
...
TEST_F(HeaderRestrictionFilterTest, RouteBlackList) {
  SetUpTest(black_config_);
  const std::string route_black_config = R"EOF(
  config:
    list:
      - headers:
          - name: user-agent
      ...
      - headers:
          - name: fake
            exact_match: fake_val
  )EOF";
  SetUpRouteTest(route_black_config);

  Http::TestRequestHeaderMapImpl limit{{"content-type", "test"},
                                       {":method", "GET"},
                                       {":authority", "www.proxy.com"},
                                       {":path", "/path"},
                                       {"fake", "fake_val"}};
  EXPECT_EQ(Http::FilterHeadersStatus::StopIteration, filter_->decodeHeaders(limit, true));

  Http::TestRequestHeaderMapImpl no_limit{{"content-type", "test"},
                                          {":method", "GET"},
                                          {":authority", "www.proxy.com"},
                                          {":path", "/path"},
                                          {"test-header-2", "test-header-2"}};
  EXPECT_EQ(Http::FilterHeadersStatus::Continue, filter_->decodeHeaders(no_limit, true));
}

TEST_F(HeaderRestrictionFilterTest, RouteWhiteList) {
  SetUpTest(white_config_);
  const std::string route_white_config = R"EOF(
  config:
    type: WHITE
    list:
      - headers:
  ...
  SetUpRouteTest(route_white_config);
...
TEST_F(HeaderRestrictionFilterTest, RouteDisableTest) {
  SetUpTest(white_config_);
  const std::string route_disable_config = R"EOF(
  disabled: true
  )EOF";
  SetUpRouteTest(route_disable_config);
