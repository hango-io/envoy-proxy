...
TEST_F(CircuitBreakerFilterTest, RtThresholdOpenWithUpstreamTiming) {
  InSequence s;
  ...
  ON_CALL(decoder_callbacks_, streamInfo()).WillByDefault(testing::ReturnRef(stream_info));

  stream_info.upstream_info_->upstreamTiming().first_upstream_tx_byte_sent_ =
      MonotonicTime(std::chrono::nanoseconds(20000));
  stream_info.upstream_info_->upstreamTiming().first_upstream_rx_byte_received_ =
      MonotonicTime(std::chrono::nanoseconds(30000));
  ...
  stream_info.upstream_info_->upstreamTiming().first_upstream_rx_byte_received_ =
      MonotonicTime(std::chrono::nanoseconds(300000000));
  ...
  ON_CALL(decoder_callbacks_, streamInfo()).WillByDefault(testing::ReturnRef(stream_info1));

  Http::TestResponseHeaderMapImpl response_headers_error{{":status", "500"}};
  EXPECT_EQ(Http::FilterHeadersStatus::Continue,
            filter_->encodeHeaders(response_headers_error, true));
  EXPECT_EQ(true, mock_manager_.impl_.isOpen(route_config_->uuid(), cb));
  EXPECT_EQ(3, breaker->consecutiveSlowRequestCount());
  ...
  now = time_system_.systemTime();
  EXPECT_EQ(0, dynamic_cast<CircuitBreakerImpl*>(_breaker)->consecutiveSlowRequestCount());
  EXPECT_EQ(1, rollingErrors.Sum(now));
  // Old buckets are still within lookback duration.
  EXPECT_EQ(9, rollingRequests.Sum(now));
...
TEST_F(CircuitBreakerFilterTest, NoRouteTest) {
...
TEST_F(CircuitBreakerFilterTest, NoRouteConfigTest) {
  setUpFilter();
