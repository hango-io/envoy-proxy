...
TEST(ExtractorRewriterConfigExtractorTest, ExtractNothing) {
  ProtoExtractor proto_config;

  EXPECT_THROW_WITH_MESSAGE(ExtractorRewriterConfig::Extractor{proto_config}, EnvoyException,
                            "Unknown source type for header rewrite extractor");
}
...
TEST(HeaderRewriteConfigTest, HeaderRewriteConfigCommonTest) {
  ...
  {
    constexpr absl::string_view error_decoder_config_with_path = R"EOF(
      decoder_rewriters:
        rewriters:
        - path: {}
    )EOF";

    TestUtility::loadFromYaml(std::string(error_decoder_config_with_path), rewriters);

    EXPECT_THROW_WITH_MESSAGE(auto error_config = HeaderRewriteConfig(rewriters), EnvoyException,
                              "Un-supported target: 2 or operation: 0 for header rewrite");
  }
  {
...
public:
  void setup(const absl::string_view common_config, const absl::string_view route_config) {
...
TEST_F(HeaderRewriteFilterTest, RewriterWithRouteConfigDisabled) {
  {
    ...
    EXPECT_EQ("baidu.com", headers1.get_(":authority"));
    EXPECT_EQ("custom-value", response_headers.get_("custom"));
  }
  {
    constexpr absl::string_view rewriter_config = R"EOF(
    config:
    ...
    EXPECT_EQ(true, response_headers.get(Http::LowerCaseString("custom")).empty());
  }
}
