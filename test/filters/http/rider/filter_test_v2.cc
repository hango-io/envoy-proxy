...
public:
  ...
  void SetUpTest(const std::string& yaml, bool allow_no_route = true) {
    ...
    std::string testlib_path =
        TestEnvironment::runfilesPath("test/filters/http/rider/testlib_v2.lua", "envoy_proxy") +
        ";";
...
TEST_F(RiderFilterTest, GetMetadataValue) {
  ...
  EXPECT_CALL(*decoder_callbacks_.route_, metadata()).WillOnce(ReturnRef(metadata));
...
TEST_F(RiderFilterTest, GetDynamicMetadataValue) {
  const std::string yaml_config = R"EOF(
  plugin:
    name: test
    code:
      local:
        inline_string: |
          ...
          function envoy_on_request()
              envoy.logInfo(envoy.req.get_dynamic_metadata("key", "envoy.test"))
              envoy.logInfo(envoy.req.get_dynamic_metadata("number", "envoy.numbertest"))
          end
  ...
  plugin:
    ...
    code:
      local:
        inline_string: |
          local testlib = require("testlib_v2")
          function envoy_on_request_header()

          end

          function envoy_on_request_body()
            local body = envoy.req.get_body()
            nil_object(foo)
          ...
          return {
            version = "v2",
            on_request_header = envoy_on_request_header,
            on_request_body = envoy_on_request_body,
          }
  )EOF";
...
TEST_F(RiderFilterTest, GetConfiguration) {
  ...
  EXPECT_CALL(*filter_, log(spdlog::level::level_enum::info, HasSubstr("{\"foo\":\"bar\"}")));
  EXPECT_EQ(Http::FilterHeadersStatus::StopIteration,
            filter_->decodeHeaders(request_headers, false));
}

TEST_F(RiderFilterTest, StartBodyWhenNoData) {
...
TEST_F(RiderFilterTest, HttpCallWhenNodata) {
  const std::string yaml_config = R"EOF(
  plugin:
    name: test
    code:
...
TEST_F(RiderFilterTest, BadArguement) {
    ...
    code:
      local:
        inline_string: |
          ...
          function envoy_on_request()
              if (envoy.req.get_header_size("") < 0) then
                envoy.logInfo("BadArguement")
              end
              if (envoy.req.get_header_index("", 1) == nil) then
                envoy.logInfo("BadArguement")
              end
              if (envoy.req.get_header_index("test", 1) == nil) then
                envoy.logInfo("NotFound")
              end
              if (envoy.req.get_dynamic_metadata("key", "fake") == nil) then
                envoy.logInfo("NotFound")
              end
              if (envoy.req.get_dynamic_metadata("fake", "envoy.test") == nil) then
                envoy.logInfo("NotFound")
              end
          end
  ...
  EXPECT_CALL(*filter_, log(spdlog::level::level_enum::info, StrEq("BadArguement"))).Times(2);
