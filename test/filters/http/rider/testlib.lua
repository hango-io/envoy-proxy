...
ffi.cdef[[
    ...
    int64_t envoy_http_lua_ffi_start_time(ContextBase *r);
...
function envoy.logTrace(message)
    C.envoy_http_lua_ffi_log(get_context(), TRACE, tostring(message))
end

function envoy.logDebug(message)
    C.envoy_http_lua_ffi_log(get_context(), DEBUG, tostring(message))
end

function envoy.logWarn(message)
    C.envoy_http_lua_ffi_log(get_context(), WARN, tostring(message))
end
