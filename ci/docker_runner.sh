...
docker run --rm ${DOCKER_RUNNING_MODE} --privileged ${DOCKER_TTY_OPTION} -e HTTP_PROXY=${http_proxy} \
    ...
    --user vscode -e BAZEL_BUILD_EXTRA_OPTIONS -e BAZEL_EXTRA_TEST_OPTIONS \
    -e BAZEL_REMOTE_CACHE -e BAZEL_REMOTE_INSTANCE -e NUM_CPUS -e ENVOY_RBE \
    --cap-add SYS_PTRACE --cap-add NET_RAW --cap-add NET_ADMIN ${ENVOY_DOCKER_IMAGE} \
