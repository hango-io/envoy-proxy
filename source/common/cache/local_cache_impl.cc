...
void LruCacheImpl::_remove(const CacheKeyType& key) {
  auto iter = m_dict_.find(key);
  if (iter == m_dict_.end()) {
    return;
  }
...
LocalCache::LocalCache(const Cache& config, Server::Configuration::FactoryContext& factory) {
  ...
  uint64_t max_cache_size = config.local().has_max_cache_size()
                                ? config.local().max_cache_size().value()
...
// Create a random number from cache key and use it to select a lru cache.
uint64_t LocalCache::_random(const CacheKeyType& key) {
  ASSERT(key.size() == 32);
  uint64_t random = 0;
  std::memcpy(&random, key.data(), 8);
  return random;
}

CacheEntryPtr LocalCache::lookupCache(const CacheKeyType& key) {
  return lru_caches_[_random(key) % cache_list_number_]->lookup(key);
}
...
void LocalCache::insertCache(const CacheKeyType& key, CacheEntryPtr&& value) {
  lru_caches_[_random(key) % cache_list_number_]->insert(key, std::move(value));
}

void LocalCache::removeCache(const CacheKeyType& key) {
  lru_caches_[_random(key) % cache_list_number_]->remove(key);
}

class LocalCacheFactory : public CommonCacheFactory {
public:
  CommonCacheBasePtr create(const Cache& config,
                            Server::Configuration::FactoryContext& factory) override {
    if (config.cache_type_case() != Cache::CacheTypeCase::kLocal) {
      return nullptr;
    }
    return std::make_unique<LocalCache>(config, factory);
  }

  std::string name() const override { return "LocalCache"; }
};

static Registry::RegisterFactory<LocalCacheFactory, CommonCacheFactory> register_;
