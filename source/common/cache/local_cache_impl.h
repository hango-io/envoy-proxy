...
private:
  void _remove(const CacheKeyType& key);
...
public:
  LocalCache(const Cache&, Server::Configuration::FactoryContext&);
...
private:
  uint64_t _random(const CacheKeyType& key);
  ...
  const uint64_t cache_list_number_{16};
