...
void DubboUpstream::writeRequestToUpstream() {
  ...
  if (dubbo_bridge == nullptr) {
    ...
    // and  cleared. It means the upstream request will be set to null when Envoy calling
...
DubboUpstream::ResponseStatus DubboUpstream::readRespnoseFromUpstream() {
  ...
  while (response_metadata_->messageType() == MessageType::HeartbeatRequest ||
    ...
    response_metadata_.reset(new Envoy::Extensions::NetworkFilters::DubboProxy::MessageMetadata());
...
void DubboUpstream::handleRawResponse(uint32_t code, absl::string_view body, bool close) {
  ...
  upstream_request_->decodeHeaders(std::move(response_headers), body.empty());
  ...
  if (close && upstream_conn_data_ != nullptr) {
    upstream_conn_data_->connection().close(Envoy::Network::ConnectionCloseType::NoFlush);
  }
}
...
void DubboUpstream::readDisable(bool disable) {
  if (upstream_conn_data_->connection().state() != Envoy::Network::Connection::State::Open) {
    return;
  }
  upstream_conn_data_->connection().readDisable(disable);
}

void DubboUpstream::resetStream() {
  // Set guard to false.
...
void DubboUpstream::onEvent(Envoy::Network::ConnectionEvent event) {
  if (event != Envoy::Network::ConnectionEvent::Connected && upstream_request_) {
